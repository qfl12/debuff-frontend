  ...
  <select v-model="selectedStatus" class="filter-select">
    ...
    <div v-else>
      <div v-if="filteredItems.length > 0" class="inventory-grid">
        <div class="item-card" v-for="item in filteredItems" :key="item.id" @mouseenter="adjustDetailPosition">
          <!-- 仅显示指定的五种磨损状态 -->
          ...
          <div class="item-info">
            ...
            <div class="item-meta">
              ...
              <span class="item-price">{{ item.price ? item.price.toFixed(2) : '上架' }}</span>
            </div>
          </div>
          <!-- 新增悬停详情面板 -->
...
const selectedStatus = ref('');
    ...
    const fetchInventory = async () => {
      ...
      errorMsg.value = '';
      try {
        const response = await service.get('/steam/inventory');
...
const getDescription = (item) => {
  ...
  if (isWeaponCase && item.descriptions) {
    return item.descriptions
      .filter(d => {
        const value = d.value?.trim();
        const name = d.name;
        return value && 
               !['blank', 'attribute'].includes(name) && 
               !(name && name.startsWith('attr:'));
      })
      .map(d => d.value.trim())
      .join('\n');
  }
